...
int main() {
	...
	for (int i =0; i<20; i++){
		cout << "randX[0..20]= " << randX[i]  << "  randY[0..20]= " << randY[i] << endl << endl;
	}
	*/
	...
	for (int iteration=0; iteration<int(n/iteration_size);iteration++){
		...
		for (int i = 0; i < iteration_size; i++) {
			randX[(iteration*iteration_size+i)] = hostRandX[i];
			randY[(iteration*iteration_size+i)] = hostRandY[i];
		}
	...
	*/
	...
	//start count cpu_omp time
	double dtime = omp_get_wtime();
	c_num = count_pi_cpu_omp(randX, randY, n);
	//end count cpu_omp time
